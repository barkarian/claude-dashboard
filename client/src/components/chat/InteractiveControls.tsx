...
import type { InteractiveState, AllowedKey } from '../../../../shared/types/interactive.ts';

interface ArrowPadProps {
  onKey: (key: AllowedKey) => void;
}

function ArrowPad({ onKey }: ArrowPadProps) {
...
interface SelectionMenuControlsProps {
  state: InteractiveState;
  onKey: (key: AllowedKey) => void;
}

function SelectionMenuControls({ state, onKey }: SelectionMenuControlsProps) {
  function handleSelect(idx: number) {
    const current = state.selectedIndex;
    const diff = idx - current;
    const direction: AllowedKey = diff > 0 ? 'ArrowDown' : 'ArrowUp';
...
function PermissionControls({ state, onText }: PermissionControlsProps) {
...
interface MultiOptionControlsProps {
  state: InteractiveState;
  onText: (text: string) => void;
}

function MultiOptionControls({ state, onText }: MultiOptionControlsProps) {
  return (
    <div className="flex flex-col gap-1.5">
...
interface SimpleConfirmationControlsProps {
  onText: (text: string) => void;
}

function SimpleConfirmationControls({ onText }: SimpleConfirmationControlsProps) {
  return (
...
function TextInputControls({ state, onText, onKey }: TextInputControlsProps) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-1.5">
      ...
      <div className="flex gap-2 items-center">
        <button
          ...
          className="flex items-center justify-center w-10 h-10 rounded-lg bg-bg-surface active:bg-bg-hover text-text transition-colors select-none touch-manipulation flex-shrink-0"
          aria-label="Up"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 15l7-7 7 7" />
...
interface InteractiveControlsProps {
  ...
  onKeyPress: (key: AllowedKey) => void;
  onTextResponse: (text: string) => void;
}

export default function InteractiveControls({ interactiveState, onKeyPress, onTextResponse }: InteractiveControlsProps) {
    ...
    <div className="px-4 py-3 border-t border-border bg-bg-main">
      <div className="flex gap-4 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          {type === 'text-input' && (
            <TextInputControls state={interactiveState} onText={onTextResponse} onKey={onKeyPress} />
          )}
        </div>
