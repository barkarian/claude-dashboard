...
/**
 * Unified buffer analysis — accepts pre-rendered lines from @xterm/headless.
 ...
 */
export function analyzeBuffer(renderedLines: string[], startupComplete: boolean): BufferAnalysis {
  // Run interactive detection first
  const interactive = startupComplete ? detectInteractiveState(renderedLines) : null;
  ...
  const sessionStatus = detectSessionStatus(renderedLines);
  return { sessionStatus, interactive: null };
}

/**
 * Detect whether Claude Code is idle (showing prompt) or thinking.
 * Accepts pre-rendered lines (already stripped of ANSI sequences by the headless terminal).
 */
function detectSessionStatus(renderedLines: string[]): SessionStatus {
  const lastLines = renderedLines.filter(l => l.trim());
