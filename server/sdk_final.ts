...
const STALE_WAITING_STATE_TIMEOUT_MS = 15 * 1000; // 15 seconds for stuck permission/tool-use states
...
function interrupt(chatId: string): void {
  ...
  // CRITICAL FIX: Explicitly emit idle status on interrupt
  // This ensures immediate UI update instead of waiting for abort error handling.
  // Without this, status can get stuck in 'waiting-permission' or 'tool-use' when user presses Escape.
  // Also touch activity to reset the stale state timeout counter.
  touchActivity(session);
  emitStatus(session, 'idle');
}
